...
这个项目真正有辨识度的，不只是文件结构，而是它打包出来的一整套 operating model：

- **存在的意义，是在预算约束下持续为核心客户创造真实价值** —— 它把 agent system 从“能不能做这个任务”拉到“这件事现在值不值得花预算做，以及能创造什么长期价值”。
- **追求单位消耗下的最高有效价值** —— north star 不是单纯多产出，而是每一单位 token、工具调用、协作步骤、人的注意力，都尽量换回最高的真实用户价值。
- **token economics 本身就是经营模型的一部分** —— token 不是空气；推理、工具、review、协作都有成本，必须讲投入产出比。
- **长期复利比一次性完成更重要** —— 目标不只是做完任务，而是持续把已验证工作沉淀成 skills、docs、scripts、templates、recurring reviews 等长期杠杆。
- **没有具体产物就不算 finished；没有验证就不算 complete** —— 完成标准取决于 deliverable 和 evidence，而不是看起来很忙、说得很像。
- **KPI 追踪是为了持续纠偏，不是为了做 dashboard 表演** —— 客户价值产出、单位消耗效率、周期时间、资产沉淀率，构成一个轻量但持续的经营校正回路。
- **治理执行，而不是替代执行** —— 这套 kit 的目标是增强现有宿主框架，而不是和它争抢执行栈。

这些思想才是这个项目真正的产品；仓库结构只是为了把它们可移植、可采用、可复用地打包出来。
